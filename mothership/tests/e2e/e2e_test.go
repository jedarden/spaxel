...
// ============================================================================
// SPAXEL-SIM TOKEN PROVISIONING PATTERN
// ============================================================================
//
// This section documents the complete token provisioning flow used by the
// spaxel-sim CSI simulator to authenticate virtual nodes against the mothership.
// Understanding this flow is essential for working with the simulator and
// for implementing similar authentication in real ESP32-S3 firmware.
//
// 1. TOKEN REQUEST (Simulator → Mothership)
//    - Endpoint: POST /api/provision
//    - Request body: {"mac":"AA:BB:CC:DD:EE:FF"} (MAC address, optional)
//    - Response: JSON provisioning payload containing:
//      {
//        "version": 1,
//        "wifi_ssid": "...",
//        "wifi_pass": "...",
//        "node_id": "uuid",                    // Generated by mothership
//        "node_token": "64-char-hex-string",  // HMAC-SHA256(installSecret, mac)
//        "ms_mdns": "spaxel",
//        "ms_port": 8080,
...
//    - Computed as: HMAC-SHA256(installSecret, normalized_mac_address)
//    - MAC normalization: uppercase, remove colons
//      Example: "aa:bb:cc:dd:ee:ff" → "AABBCCDDEEFF"
//    - Install secret sources (priority order):
//      1. SPAXEL_INSTALL_SECRET environment variable (64-char hex)
//      2. Persisted secret in database (auth.install_secret column)
//      3. Auto-generated on first run (printed once to stdout)
//
// 3. TOKEN USAGE (WebSocket Connection)
//    - Header: X-Spaxel-Token: <64-char-token>
//    - Presented during WebSocket upgrade to /ws/node
//    - Server extracts header into hello.Token (see ingestion/server.go)
//    - Validation uses constant-time comparison to prevent timing attacks
//
// 4. AUTHENTICATION FLOW (Simulator Lifecycle)
//    a) resolveTokens() calls provisionNodeToken() for each virtual node
//    b) provisionNodeToken() POSTs to /api/provision with MAC address
//    c) Mothership derives token via deriveToken(MAC) and returns in payload
//    d) Token stored in VirtualNode.Token struct
//    e) WebSocket dial includes header: reqHeader.Set("X-Spaxel-Token", n.Token)
//    f) Ingestion server validates token via ValidateToken(mac, token)
//    g) Valid tokens accepted REGARDLESS of migration window (window-independent)
//    h) Invalid tokens rejected with 401 Unauthorized (or 403 Forbidden)
//
// 5. MIGRATION WINDOW INDEPENDENCE
//    - Real HMAC tokens are window-independent: valid tokens are accepted
//      even when SPAXEL_MIGRATION_WINDOW_HOURS=0 (strict mode)
//    - Dummy tokens (--provision=false) only work while migration window is open
//    - This allows e2e tests to run under strict window without false negatives
//
// 6. SIMULATOR FLAGS
//    - --provision (default): Mint real HMAC tokens via /api/provision
//    - --provision=false: Use dummy tokens (legacy, window-dependent)
//    - --token <t>: Apply explicit token verbatim to all nodes
//    - --mothership <url>: WebSocket endpoint URL
//
// 7. KEY FILES
//    - cmd/sim/main.go: Simulator CLI with provisioning logic
//    - mothership/internal/provisioning/server.go: /api/provision endpoint
//    - mothership/internal/ingestion/server.go: Token validation on WS upgrade
//
// 8. EXAMPLE FLOW (from cmd/sim/main.go)
//    // 1. Generate MAC for virtual node
//    mac := generateMAC(nodeID)  // "02:53:AC:00:00:01"
//    // 2. Call provisioning endpoint
//    token, err := provisionNodeToken(ctx, apiBase, macToString(mac))
//    //    POST http://localhost:8080/api/provision
//    //    Body: {"mac":"02:53:AC:00:00:01"}
//    //    Response: {"node_token":"a1b2c3..."} (64-char hex)
//    // 3. Store token
//    node.Token = token
//    // 4. Connect via WebSocket with token header
//    reqHeader.Set("X-Spaxel-Token", token)
//    conn, _, err := websocket.DefaultDialer.DialContext(ctx, u.String(), reqHeader)
//
// 9. SECURITY CONSIDERATIONS
//    - Constant-time comparison prevents timing attacks
//    - Per-node tokens bind authentication to specific hardware MAC
//    - Install secret is fleet-wide credential (protect appropriately)
//    - Tokens transmitted in clear over HTTP (use HTTPS in production)
//    - Dummy tokens are cryptographically weak and only for development
//
// For implementation details, see:
//   - cmd/sim/main.go:374-408 (provisionNodeToken function)
//   - mothership/internal/provisioning/server.go:248-256 (deriveToken)
//   - mothership/internal/provisioning/server.go:180-190 (ValidateToken)
//   - mothership/internal/provisioning/server.go:193-275 (HandleProvision)
//   - mothership/internal/ingestion/server.go:594-618 (token validation)
// ============================================================================
