...
1. **Store WiFi credentials in the mothership's `settings` table, editable through the dashboard at runtime — not an environment variable.** Add a `NetworkSettingsHandler` following the `NotificationSettingsHandler` pattern exactly: `GET`/`PUT /api/settings/network`, typed request/response structs, persisted via the existing `settings` table. A new "Network" section in the dashboard's Settings page lets the operator enter the fleet's WiFi SSID/password once, and change it later, without editing a deployment manifest or restarting the mothership.
...
3. **Fail loudly, not silently, when neither source has credentials.** No stored setting and no request-body override means the endpoint returns an error rather than provisioning a node with an empty or garbage SSID that will never join anything. The dashboard's own onboarding wizard should not be able to reach the flash step at all without one or the other being satisfied first.
...
5. **The dashboard onboarding wizard skips the WiFi step when a stored setting exists**, surfacing it only under an "Advanced: use a different network for this node" affordance. This removes an entire step — and the single most repeated point of friction found during the bench session — from the common path.
