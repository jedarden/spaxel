...
3. Mothership generates a provisioning payload: WiFi SSID, WiFi password, unique node ID. **The WiFi network is not asked per device** — every node is presumed to join the same fleet network, so the mothership fills in `wifi_ssid`/`wifi_pass` from a fleet-wide setting configured once in the dashboard's **Settings > Network** page (`GET`/`PUT /api/settings/network`; see ADR-005), rather than prompting the user to type them in for each node. The onboarding wizard displays the fleet status and has no per-device WiFi fields; direct API callers may explicitly override the fleet setting for an exceptional node. **No mothership IP needed** — firmware discovers mothership via mDNS (`_spaxel._tcp.local`)
...
- If a node loses WiFi, captive portal at `spaxel-XXXX` allows re-entering credentials. The captive portal itself still requires manual entry (the device has no route to the mothership while its setup AP is up — see ADR-005's Alternatives Considered), but the dashboard's "recover via captive portal" screen displays the Settings > Network credentials in copy-paste-ready form before the user switches networks, so the fleet password doesn't need to be remembered or looked up separately
...
**Source of `wifi_ssid`/`wifi_pass` (ADR-005):** by default these come from the mothership's stored `network` setting (Settings > Network in the dashboard, `GET`/`PUT /api/settings/network`), not the request body — every node is presumed to join the same fleet network, configured once through the UI rather than re-entered per device. `SPAXEL_WIFI_SSID`/`SPAXEL_WIFI_PASSWORD` env vars can seed this setting on first boot for scripted deployments, but the stored setting is authoritative after that. The request body may still supply `wifi_ssid`/`wifi_pass` explicitly to override this for a specific node (e.g., a node that genuinely belongs on a different network); an explicit value in the request always wins. If neither the stored setting nor the request body supplies credentials, the endpoint returns an error rather than provisioning a node with no network to join.
...
| `SPAXEL_WIFI_SSID` | *(unset)* | Optional **first-boot only** seed for the fleet's WiFi network name (see ADR-005). If set and no `network` setting exists yet in the DB, seeds it once; ignored on every subsequent boot. The actual, editable-anytime source of truth is Settings > Network in the dashboard (`PUT /api/settings/network`), not this variable — use the UI to change it later, not a redeploy |
| `SPAXEL_WIFI_PASSWORD` | *(unset)* | First-boot seed for the passphrase, same rule as `SPAXEL_WIFI_SSID` |
...
5. Publish a real multi-arch `ronaldraygun/spaxel:$(cat VERSION)` manifest list. The `spaxel-build` WorkflowTemplate's `docker-build` step currently builds with the **Kaniko** executor (`gcr.io/kaniko-project/executor`), which has no native `--platform a,b` multi-arch output the way `docker buildx build` does. Concretely this means either (a) invoking Kaniko once per target arch and stitching the results into a manifest list with a separate `docker manifest create`/push (or `manifest-tool`) step, or (b) migrating that step to a BuildKit-based builder that supports `buildx` natively. (b) is the more standard, maintainable path and is the one assumed by "Consequences" below; it is a larger CI-infra change than (a) and is broken out as its own implementation step rather than assumed to be free.
...
The mothership already has exactly the infrastructure this belongs on: a generic `settings` table (key, `value_json`) behind `internal/api/settings.go`'s `SettingsHandler` (`GET`/`POST /api/settings`), and a domain-specific wrapper pattern already proven on top of it — `NotificationSettingsHandler` (`internal/api/notification_settings.go`) exposes `GET`/`PUT /api/settings/notifications` with its own typed request/response structs, backed by the same shared table. Fleet WiFi is the same shape of problem notifications already solved: a small set of fleet-wide settings, edited occasionally through the dashboard, that need to survive restarts and be readable by other subsystems.

### Decision

1. **Store WiFi credentials in the mothership's `settings` table, editable through the dashboard at runtime.** Add a `NetworkSettingsHandler` following the `NotificationSettingsHandler` pattern exactly: `GET`/`PUT /api/settings/network`, typed request/response structs, persisted via the existing `settings` table. A new "Network" section in the dashboard's Settings page lets the operator enter the fleet's WiFi SSID/password once, and change it later, without editing a deployment manifest or restarting the mothership. `SPAXEL_WIFI_SSID` and `SPAXEL_WIFI_PASSWORD` are optional first-boot seeds for headless deployments; the database remains authoritative afterward.
2. **`POST /api/provision` defaults to the stored setting.** If the request body omits `wifi_ssid`/`wifi_pass`, the mothership fills them in from the `network` settings entry. An explicit value in the request body still overrides this — a node that genuinely belongs on a different network (a detached sensor on its own AP, a test rig) remains fully supported, it is just no longer the assumed case.
3. **Warn clearly when neither source has credentials.** No stored setting and no request-body override produces an empty WiFi section with a warning, allowing open-network and captive-portal flows while making the missing fleet configuration visible in the dashboard.
4. **`SPAXEL_WIFI_SSID` / `SPAXEL_WIFI_PASSWORD` environment variables remain, but only as an optional first-boot seed** — useful for a fully scripted/headless deployment that wants the network pre-configured before anyone opens the dashboard. On first boot, if set and no `network` setting exists yet in the DB, they seed it once. After that the DB row is the source of truth; the env var is not re-read or re-applied, so editing it after first boot does nothing (avoiding the surprise of a manifest change silently overriding a value someone since changed through the UI).
5. **The dashboard onboarding wizard displays the fleet network status without collecting WiFi credentials.** This removes the repeated per-device data entry that caused friction during the bench session while keeping the provisioning step visible as an operator checkpoint. Exceptional networks remain available through explicit direct API overrides.
6. **The captive portal keeps manual entry**, because it has no way to be anything else: while a device's own `spaxel-XXXX` setup AP is up, the host connected to it has, by construction, no route to the mothership to fetch stored credentials from. What changes is that the dashboard's own "recover via captive portal" screen displays the stored credentials in copy-paste-ready form *before* the user switches networks, so the fleet password only has to live in one place (the mothership's Settings page) instead of also in the operator's memory or a side channel.
7. **No firmware or NVS schema change.** This is entirely a mothership + dashboard change in *where* `wifi_ssid`/`wifi_pass` originate before being bundled into the same provisioning payload the device has always received. `provision.c`, the NVS keys, and the Web Serial / captive-portal wire protocols are untouched.

### Alternatives Considered

- **Environment-variable-only configuration, set at mothership deploy time.** This was the first-drafted version of this ADR, rejected on review. It matches the existing `SPAXEL_NTP_SERVER` pattern, but NTP server and WiFi credentials are not actually the same *kind* of setting: NTP server is infrastructure-operator config, chosen once when standing up the deployment; the fleet WiFi password is something a non-technical end user sets up during initial setup and may need to change later (a router password rotation, a network migration) without knowing what a Kubernetes manifest is or wanting to trigger a redeploy + ArgoCD sync to change one string. This project's own stated onboarding goals — "zero-config first run," a guided wizard, "working presence detection in under 5 minutes" — describe a home-user product, not an infrastructure-operator one; a setting the target user is expected to change occasionally belongs in the UI they already use, not in a file they'd need cluster access to edit. The mothership already has a real, working settings API for exactly this class of value (see Context) — using it instead of a new env var isn't extra work, it's the smaller change.
- **Push credentials to the device directly from the mothership, bypassing the browser form entirely, for the captive-portal path too.** Rejected as infeasible, not merely undesirable: the mothership cannot reach a device's temporary setup AP (different network, and the whole point of the captive portal is that the device has no other connectivity yet), and the browser doing the provisioning cannot be on both networks simultaneously either. The realistic improvement is eliminating *retyping*, not eliminating the network switch — hence decision 6's copy-paste affordance rather than a fully automatic captive-portal path.
...
- **Drop the per-request override and make the stored setting the only source.** Rejected. A minority of real deployments legitimately need a node on a different network (an isolated test rig, a sensor bridging two sites), and removing the override would regress that case to force a settings change just to onboard one outlier node.
...
- **Positive:** The fleet's WiFi password can be changed (router rotation, network migration) from the dashboard in seconds, with no redeploy, no ArgoCD sync, no mothership restart — the actual reason this belongs in the settings table instead of an env var.
- **Positive:** Reuses proven infrastructure instead of adding new infrastructure — `NetworkSettingsHandler` is a straightforward sibling of the already-shipped `NotificationSettingsHandler`, not a novel mechanism.
- **Cost:** A new API handler + dashboard Settings section to build (`NetworkSettingsHandler`, following `NotificationSettingsHandler` as a template), plus wizard conditional logic (skip-if-configured, "Advanced" override) and the captive-portal recovery screen's copy-paste display.
...
- **Follow-up:** `bf-1j1ws` (versioned app-only seed artifact), `bf-4jgpd` (trigger path filter — **done**, landed 2026-08-07 on the argo-events sensor in declarative-config and verified live 2026-08-20; see `docs/notes/ci-doc-only-push-path-filter.md`) and `bf-5fr9b` (canary rollback actually reverts) are all wired as blockers on `bf-1tcxc` (enable auto-update + drift alerting), so the enable step cannot land before the safety net exists.
