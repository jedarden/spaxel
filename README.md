...
- **Rough person count** — distinguishes 1 vs. 2+ (degrades at 3+). *Met at the re-based gate* (AS-9: a single walker presents as exactly 1 blob — median 1.0, so the earlier "median 6" over-segmentation is fixed; two walkers are both detected — ≥ 1 blob in 100 % of polls — and simultaneous ≥ 2-blob separation demonstrably occurs, though intermittently (~18–20 % of polls), because the engine's Fresnel-zone merging sums nearby walkers into one ridge. The original "≥ 2 blobs for ≥ 50 % of the run" gate over-stated what the pipeline delivers and was re-based, together with this claim, to presence + separation existence — map §4 C3)
